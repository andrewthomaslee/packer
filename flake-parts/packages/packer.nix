    ...
    mkHcloudPkrJson = system: server_type:
      pkgs.writeText "hcloud-${system}.pkr.json" (builtins.toJSON (lib.recursiveUpdate hcloud {
        ...
        build.provisioner = [
          ...
          {
            shell-local.inline = [
              "clan machines install hcloud-${system} --target-host \${build.User}@\${build.Host} --update-hardware-config nixos-generate-config -i \${var.temp_priv_key} --host-key-check none --yes"
            ];
          }
        ];
      }));
